...
class AppAuthenticator extends AbstractLoginFormAuthenticator
{
    ...
    public function onAuthenticationSuccess(Request $request, TokenInterface $token, string $firewallName): ?Response
    {
        ...
        if($this->authorizationChecker->isGranted("ROLE_TECNICO")){
            return new RedirectResponse($this->urlGenerator->generate('app_tecnico_dashboard'));
        }
        if($this->authorizationChecker->isGranted("ROLE_FACTURACION")){
            return new RedirectResponse($this->urlGenerator->generate('app_facturacion_dashboard'));
        }
        // For example:
